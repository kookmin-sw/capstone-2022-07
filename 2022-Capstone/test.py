...
from multiprocessing import Pool,  Manager  # Pool import하기
...
def search_crawl(result, urll):
    original_html = requests.get(urll)
...
def run():
    ...
    pool.starmap(search_crawl, [(result, url) for url in urls])
    pool.close()
    pool.join()
