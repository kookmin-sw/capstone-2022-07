...
#엑셀로 저장하기 위한 변수
RESULT_PATH ="C:/Users/LeeSeungJun/Desktop/coding/capstone/"
#결과 저장할 경로
...
def crawler(title_list,url_list,result_dict, query):
    ...
    while page <= maxpage_t:
        url = "https://search.naver.com/search.naver?where=news&query=" + query + "&sort="+str(sort)+"&pd=4&ds=" + s_date + "&de=" + e_date + "&nso=so%3Ar%2Cp%3A&start=" + str(page)
...
def run():
    ...
    df = pd.read_csv('li.csv', encoding='cp949')
    company = df.values.tolist()
    company = sum(company, [])
    ...
    pool.starmap(crawler, [(title_list,url_list,result_dict, query) for query in cospi])
    pool.close()
    pool.join()

    end = time.time()
    dict = {"title":title_list, "urls":url_list,}
    print(dict["title"])
    ...
    #df = pd.DataFrame(dict, index = [cospi])
    outputFileName = '%s-%s-%s  %s시 %s분 %s초 merging.xlsx' % (now.year, now.month, now.day, now.hour, now.minute, now.second)
    #df.to_excel(RESULT_PATH+outputFileName,sheet_name='sheet1')
